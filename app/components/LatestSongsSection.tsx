...
const LatestSongsSection: FC<SearchComponentProps> = ({
  SearchCartTitle,
  SearchCardData,
}) => {
  ...
  //   router.push(`latestsongdetail?id=${id}`);
  // };
    ...
    <div className="h-[480px] custom-container">
          ...
          <Slider ref={sliderRef} {...settings} className="h-60">
            {SearchCardData.map((item) => (
             <Link key={item.id} href={`/music/${item.id}/${item.title}`}>
              <CommonVerticalcard                
                img={item.img}
                title={item.title}
                ...
                description2={`${item.duration} | ${item.quality}`}

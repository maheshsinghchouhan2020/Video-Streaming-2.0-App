...
import newlyAddedVideoData from "@/app/content/shortSectionData/kannadaWebseriesData";
import Link from "next/link";

const KannadaWebseries = () => {
  ...
  useEffect(() => {
    const updateArrowButtonsVisibility = () => {
      const totalItems = newlyAddedVideoData.length;
  ...
  return (
    <div className="custom-container">
      ...
      <Slider ref={sliderRef} {...settings} className="h-60">
       ...
       <Link key={item.id} href={`/shorts/${item.id}/${item.title}`}>
...
export default KannadaWebseries;
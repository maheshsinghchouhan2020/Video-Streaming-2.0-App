...
const HeroSectionDetailHome = () => {
  ...
  const ranges = Array.from(
    { length: Math.ceil(homeBannerMovies.length / 5) },
    (_, index) => {
      const start = index * 5;
      return { start, end: Math.min(start + 5, homeBannerMovies.length) };
    }
  );
  ...
  const filteredData = useMemo(() => {
    return homeBannerMovies.slice(visibleRange[0], visibleRange[1]);
  }, [visibleRange]);

  useEffect(() => {
    const fetchMovieDetails = () => {
      if (id) {
        const selectedMovie = homeBannerMovies.find(
...
export default HeroSectionDetailHome;

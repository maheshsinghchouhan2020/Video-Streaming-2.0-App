...
import { ShortSectionData } from "../content/shortSectionData/ShortBannerData";
...
const ShortsBannerDetail = () => {
  ...
  const ranges = Array.from(
    { length: Math.ceil(ShortSectionData.length / 5) },
    (_, index) => {
      const start = index * 5;
      return { start, end: Math.min(start + 5, ShortSectionData.length) };
    }
  );
  ...
  const filteredData = useMemo(() => {
    return ShortSectionData.slice(visibleRange[0], visibleRange[1]);
  }, [visibleRange]);

  useEffect(() => {
    const fetchMovieDetails = () => {
      if (id) {
        const selectedMovie = ShortSectionData.find(
  ...
  return (
    <div>
      <CommonCardDetailView
        heading={
          <Link href="/shorts" className="hover:underline hover:text-white">
            Shorts
          </Link>
        }
...
export default ShortsBannerDetail;